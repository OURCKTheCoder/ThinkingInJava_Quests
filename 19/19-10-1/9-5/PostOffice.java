...
class Mail {
  ...
  public static Mail randomMail() {
    ...
    m.isRepost = Enums.random(Repost.class);
    return m;
  }
...
public class PostOffice {
  enum MailHandler {
    ...
    CHECK_REPOST {
		...
		boolean handle(Mail m) {
			switch(m.isRepost) {
			case YES: print("Reposting..."); return true;
			default: return false;
			}
		}
    },
  ...
  static void handle(Mail m) {
	EnumMap<MailHandler, Boolean> processResult = new EnumMap<>(MailHandler.class);
    for(MailHandler handler : MailHandler.values()) {
    	processResult.put(handler, handler.handle(m));
    	if(processResult.get(handler))
    		return;
    }
    print(m + " is a dead letter");
  }
  public static void main(String[] args) {
    for(Mail mail : Mail.generator(50)) {
      print(mail.details());
      handle(mail);
      print("*****");
    }
